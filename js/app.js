...
function createLanguageSelect() {
    removeElements($$("#language-dropdown a"));
    removeElements($$("#language-dropdown hr"));

    if (languageHelper.getPresentLanguages().length > 0) {
        for (let locale of languageHelper.getPresentLanguages()) {
            let option = document.createElement("a");
            option.className = "navbar-item";
            option.textContent = languageHelper.getLanguageNameByLocale(locale);
            option.setAttribute("locale", locale);
            option.onclick = clickEvent => changeLanguage(clickEvent.target.getAttribute("locale"));
            if (locale == languageHelper.getCurrentLocale()) {
                $("#current-language").textContent = languageHelper.getLanguageNameByLocale(locale);
            }
    ...
    for (let locale of languageHelper.getNonPresentLanguages()) {
        let option = document.createElement("a");
        option.className = "navbar-item";
        option.textContent = languageHelper.getLanguageNameByLocale(locale);
        option.setAttribute("locale", locale);
...
function createConditionSelect() {
    removeElements($$("#condition-select-outer"));
...
function createMeasurementUnitSelect() {
    removeElements($$("#measurement-unit-select-outer"));
...
function hideForms(hideTree) {
    removeElements($$("#form-panel-blocks a"));
...
function hideItemGroups(hideTree) {
    removeElements($$("#item-group-panel-blocks a"));
...
function hideItems(hideTree) {
    removeElements($$("#item-panel-blocks a"));
...
function hideCodeListItems(hideTree) {
    removeElements($$("#code-list-item-panel-blocks a"));
...
function loadStudyEvents() {
    ...
    removeElements($$("#study-event-panel-blocks a"));
...
function fillRangeChecks() {
    removeElements($$(".range-check-input"));
...
function fillConditions() {
    removeElements($$(".condition-input"));
...
function fillMeasurementUnits() {
    removeElements($$(".measurement-unit-input"));
...
window.toggleMode = function() {
    $("#model-section").classList.toggle("is-hidden");
    $("#capture-section").classList.toggle("is-hidden");
    $("#model-toggle-button").classList.toggle("is-hidden");
    $("#capture-toggle-button").classList.toggle("is-hidden");
}
...
window.saveElement = function() {
    ...
    fillDetailsPanel(getCurrentElementOID(), currentElementType);
...
function setIOListeners() {
    ...
    $("#current-language").addEventListener("click", () => {
        $("#language-dropdown").classList.toggle("is-hidden-mobile");
    });
    ...
    for (let [index, inputElement] of inputElements.entries()) {
        inputElement.oninput = inputDidChange;
        inputElement.onkeydown = inputElementKeyboardListener;
...
function inputElementKeyboardListener(event) {
...
window.hideDownloadModal = function() {
    $("#download-modal").classList.remove("is-active");
    setArrowKeyListener();
}
...
function changeLanguage(selectedLocale) {
    ...
    $("#language-dropdown").classList.add("is-hidden-mobile");
    languageHelper.setCurrentLanguage(locale);
    reloadTree();
